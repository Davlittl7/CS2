...
AST::~AST() {
    ...
    //Recursively travels through the tree to delete/destruct it
...
AST::AST(const AST& actual) {
    ...
    //Copies the actual version of these variables into the original piece
...
void AST::swap(AST& b) {
    //NEED TO IMPLEMENT
    //This piece allows the topmost children to be swapped
    ...
    std::string stringTmpClose = closeTag;
    closeTag = b.closeTag;
    b.closeTag = stringTmpClose;
...
void AST::mainHeader(const std::vector<std::string>& profileName) {
    ...
    //setting up strings and vectors and then inserting the include
    std::vector<std::string> newVector = profileName;
    std::string astStr = "#include \"profile.hpp\"\n";
    child.insert(i, new AST(token, astStr));
    std::vector<std::string>::iterator a = newVector.begin();
    std::vector<std::string>::const_iterator b = profileName.begin();

    //Another while loop to continue inserts 
    while (a != newVector.end()) {
        size_t tmp = (a)->find("_cpp"); //fixed it
        std::string fileName = (a)->replace(tmp, 1, ".");
        std::string objectName = "profile " + *b + "(\"" + *a + "\");" + "\n";
...
void AST::fileHeader(const std::string& profileName) {
    ...
    //Set up and initialize astString and inserts it into the file for exteral declaration
    std::string astString = "#include \"profile.hpp\"\n";
    child.insert(i, new AST(token, astString));
    std::string externalName = "extern profile " + profileName + ";" + "\n\n";
    child.insert(i, new AST(token, externalName));
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    while (!foundMain) {
        if ((*it)->tag != "function") { ++it; }
        else {
            if ((*it)->getChild("name")->child.front()->text != "main") { ++it; }
            else { foundMain = true; }
        }
    }
    std::list<AST*>::iterator itTwo = (*it)->getChild("block")->child.end();
    --itTwo;
    while ((*itTwo)->tag != "return") { --itTwo; }
    for (size_t i = 0; i < profileName.size(); ++i) { //inserts the console outputs
        child.insert(itTwo, new AST(token, "std::cout << " + profileName[i] + " << std::endl;\n"));
    }
}
...
void AST::lineCount(const std::string& profileName) {
    ...
    while (i != child.end()) {
        if (!(*i)->child.empty()) {
            (*i)->lineCount(profileName); //Recursive call to count lines 
        }
        if ((*i)->tag == "expr_stmt") {
            if (!isStopTag((*i)->tag))
